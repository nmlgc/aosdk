...
EXPORT_GCC void CALLBACK SPU2write(unsigned long reg, unsigned short val)
{
	...
	if((r>=0x0000 && r<0x0180)||(r>=0x0400 && r<0x0580)) { // some channel info?
		...
		case 6: {
			...
			s_chan[ch].ADSR.SustainLevel=(1024*((lval) & 0x000f))/15;

			// decay:
			// our const decay value is time it takes from 100% to 0% of volume
			lx=(lval>>4) & 0x000f;
			if(lx) {
			...
			// so calc how long does it take to run from 100% to the wanted sus level
			s_chan[ch].ADSR.DecayTime = (lx*(1024-s_chan[ch].ADSR.SustainLevel))/1024;
		}
		break;
		...
		case 8: {
			...
			// sustain time... often very high values are used to hold the
			// volume until a sound stop occurs
			lx=((((lval>>6) & 0x007f)>>2));
			lx=min(31,lx);
			if(lx) {
				// the highest value we reach (due to overflow checking) is:
				// 94704 seconds = 1578 minutes = 26 hours...
				// should be enuff... if the stop doesn't
				lx = (1<<lx);
				if(lx<2147483) {
			...
			// release time from 100% to 0%
			// note: the release time will be adjusted when a stop is coming,
			// so at this time the adsr vol will run from (current volume) to 0%
			if(lx) {
				lx = (1<<lx);
				if(lx<2147483) {
	...
	case PS2_C0_Reverb+86:
		...
		break;
	case PS2_C0_ReverbX+0:
		rvb[0].IIR_ALPHA=(short)val;
		break;
	case PS2_C0_ReverbX+2:
		rvb[0].ACC_COEF_A=(short)val;
		break;
	case PS2_C0_ReverbX+4:
		rvb[0].ACC_COEF_B=(short)val;
		break;
	case PS2_C0_ReverbX+6:
		rvb[0].ACC_COEF_C=(short)val;
		break;
	case PS2_C0_ReverbX+8:
		rvb[0].ACC_COEF_D=(short)val;
		break;
	case PS2_C0_ReverbX+10:
		rvb[0].IIR_COEF=(short)val;
		break;
	case PS2_C0_ReverbX+12:
		rvb[0].FB_ALPHA=(short)val;
		break;
	case PS2_C0_ReverbX+14:
		rvb[0].FB_X=(short)val;
		break;
	case PS2_C0_ReverbX+16:
		rvb[0].IN_COEF_L=(short)val;
		break;
	case PS2_C0_ReverbX+18:
		rvb[0].IN_COEF_R=(short)val;
		break;
	//-------------------------------------------------//
	...
	case PS2_C1_Reverb+86:
		...
		break;
	case PS2_C1_ReverbX+0:
		rvb[1].IIR_ALPHA=(short)val;
		break;
	case PS2_C1_ReverbX+2:
		rvb[1].ACC_COEF_A=(short)val;
		break;
	case PS2_C1_ReverbX+4:
		rvb[1].ACC_COEF_B=(short)val;
		break;
	case PS2_C1_ReverbX+6:
		rvb[1].ACC_COEF_C=(short)val;
		break;
	case PS2_C1_ReverbX+8:
		rvb[1].ACC_COEF_D=(short)val;
		break;
	case PS2_C1_ReverbX+10:
		rvb[1].IIR_COEF=(short)val;
		break;
	case PS2_C1_ReverbX+12:
		rvb[1].FB_ALPHA=(short)val;
		break;
	case PS2_C1_ReverbX+14:
		rvb[1].FB_X=(short)val;
		break;
	case PS2_C1_ReverbX+16:
		rvb[1].IN_COEF_L=(short)val;
		break;
	case PS2_C1_ReverbX+18:
		rvb[1].IN_COEF_R=(short)val;
		break;
	}
...
EXPORT_GCC unsigned short CALLBACK SPU2read(unsigned long reg)
{
	...
	if((r>=0x0000 && r<0x0180)||(r>=0x0400 && r<0x0580)) { // some channel info?
		...
		case 10: {
			...
			// same here... we haven't decoded one sample yet, so no envelope yet.
			// return 1 as well
...
//      spuCtrl=val;
	...
	*/
	...
	case H_Reverb+2:
		rvb[0].FB_SRC_B=(s16)val;
		break;
	case H_Reverb+4:
		rvb[0].IIR_ALPHA=(s16)val;
		break;
	case H_Reverb+6:
		rvb[0].ACC_COEF_A=(s16)val;
		break;
	case H_Reverb+8:
		rvb[0].ACC_COEF_B=(s16)val;
		break;
	case H_Reverb+10:
		rvb[0].ACC_COEF_C=(s16)val;
		break;
	case H_Reverb+12:
		rvb[0].ACC_COEF_D=(s16)val;
		break;
	case H_Reverb+14:
		rvb[0].IIR_COEF=(s16)val;
		break;
	case H_Reverb+16:
		rvb[0].FB_ALPHA=(s16)val;
		break;
	case H_Reverb+18:
		rvb[0].FB_X=(s16)val;
		break;
	case H_Reverb+20:
		rvb[0].IIR_DEST_A0=(s16)val;
		break;
	case H_Reverb+22:
		rvb[0].IIR_DEST_A1=(s16)val;
		break;
	case H_Reverb+24:
		rvb[0].ACC_SRC_A0=(s16)val;
		break;
	case H_Reverb+26:
		rvb[0].ACC_SRC_A1=(s16)val;
		break;
	case H_Reverb+28:
		rvb[0].ACC_SRC_B0=(s16)val;
		break;
	case H_Reverb+30:
		rvb[0].ACC_SRC_B1=(s16)val;
		break;
	case H_Reverb+32:
		rvb[0].IIR_SRC_A0=(s16)val;
		break;
	case H_Reverb+34:
		rvb[0].IIR_SRC_A1=(s16)val;
		break;
	case H_Reverb+36:
		rvb[0].IIR_DEST_B0=(s16)val;
		break;
	case H_Reverb+38:
		rvb[0].IIR_DEST_B1=(s16)val;
		break;
	case H_Reverb+40:
		rvb[0].ACC_SRC_C0=(s16)val;
		break;
	case H_Reverb+42:
		rvb[0].ACC_SRC_C1=(s16)val;
		break;
	case H_Reverb+44:
		rvb[0].ACC_SRC_D0=(s16)val;
		break;
	case H_Reverb+46:
		rvb[0].ACC_SRC_D1=(s16)val;
		break;
	case H_Reverb+48:
		rvb[0].IIR_SRC_B1=(s16)val;
		break;
	case H_Reverb+50:
		rvb[0].IIR_SRC_B0=(s16)val;
		break;
	case H_Reverb+52:
		rvb[0].MIX_DEST_A0=(s16)val;
		break;
	case H_Reverb+54:
		rvb[0].MIX_DEST_A1=(s16)val;
		break;
	case H_Reverb+56:
		rvb[0].MIX_DEST_B0=(s16)val;
		break;
	case H_Reverb+58:
		rvb[0].MIX_DEST_B1=(s16)val;
		break;
	case H_Reverb+60:
		rvb[0].IN_COEF_L=(s16)val;
		break;
	case H_Reverb+62:
		rvb[0].IN_COEF_R=(s16)val;
		break;
	}
}
...
void ReverbOn(int start,int end,unsigned short val,int iRight)
{
	...
	for(ch=start; ch<end; ch++,val>>=1) { // loop channels
		if(val&1) { // -> reverb on/off
			if(iRight) {
				s_chan[ch].bReverbR=1;
			} else {
				s_chan[ch].bReverbL=1;
			}
		} else {
			if(iRight) {
				s_chan[ch].bReverbR=0;
			} else {
				s_chan[ch].bReverbL=0;
			}
		}
	}
}
...
void VolumeOn(int start,int end,unsigned short val,int iRight)
{
	...
	for(ch=start; ch<end; ch++,val>>=1) { // loop channels
		if(val&1) { // -> reverb on/off
			if(iRight) {
				s_chan[ch].bVolumeR=1;
			} else {
				s_chan[ch].bVolumeL=1;
			}
		} else {
			if(iRight) {
				s_chan[ch].bVolumeR=0;
			} else {
				s_chan[ch].bVolumeL=0;
			}
		}
	}
}
