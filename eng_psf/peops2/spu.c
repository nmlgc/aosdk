...
{
	// flag == 1? calc step and set flag... and don't change the value in this pass
	...
	// no flags? add bigger val (if possible), calc smaller step, set flag1
	} else {
		s_chan[ch].SB[29]+=s_chan[ch].SB[28];
	}
}
...
INLINE void StartSound(int ch)
{
	...
	if(iUseInterpolation>=2) { // gauss interpolation?
		s_chan[ch].spos=0x30000L; // -> start with more decoding
		s_chan[ch].SB[28]=0;
	} else {
		s_chan[ch].spos=0x10000L; // -> no/simple interpolation starts with one 44100 decoding
		s_chan[ch].SB[31]=0;
	}
}
...
EXPORT_GCC int CALLBACK SPU2sample(stereo_sample_t *sample)
{
	...
	// while(!bEndThread) { // until we are shutting down
		...
		if(dwNewChannel2[0] || dwNewChannel2[1]) { // new channel should start immedately?
			...
			// (if it is set 5 times - that means on 5 tries a new samples has
			//  been started - in a row, we will reset it, to give the sound
			//  update a chance)
			if(iSecureStart>5) {
				iSecureStart=0;
			}
...
#if 0
		// no new start? no thread end?
		// and still enuff data in sound buffer?
		while(!iSecureStart && !bEndThread /* && (SoundGetBytesBuffered()>TESTSIZE) */) {
			iSecureStart=0; // reset secure
					...
					if(s_chan[ch].iSBPos==28) { // 28 reached?
						...
						if(spuCtrl2[ch/24]&0x40) { // some irq active?
							if((pSpuIrq[ch/24] >  start-16 && // irq address reached?
								...
								if(irqCallback) {
									irqCallback(); // -> call main emu (not supported in SPU2 right now)
								} else {
									if(ch<24) {
										InterruptDMA4(); // -> let's see what is happening if we call our irqs instead ;)
									} else {
										InterruptDMA7();
									}
								}
						...
						// special return for "spu irq - wait for cpu action"
						if(bIRQReturn) {
							bIRQReturn=0;
							lastch=ch;
...
GOON:
					...
					if(iUseInterpolation>=2) { // gauss/cubic interpolation
						...
						// -> helpers for simple linear interpolation: delay
						// real val for two slots, and calc the two deltas,
						// for a 'look at the future behaviour'
						s_chan[ch].SB[28] = 0;
				...
				if(s_chan[ch].bFMod==2) { // fmod freq channel
					...
					// mmm... I still need to adjust that to 1/48 khz... we will
					// wait for the first game/demo using it to decide how to do it :)
...
static void SetupStreams(void)
{
	...
	for(i=0; i<MAXCHAN; i++) { // loop sound channels
		// we don't use mutex sync... not needed, would only slow us down:
